...
def get_mac_data(dev, fpc_list, log_name):
    ...
    for fpc in fpc_list:
        cmd_ukern_mac = 'request pfe execute command "show l2 manager mac-table" target fpc{0}'.format(fpc.slot)
        mac_logs += ("#" * 80) + '\n' + cmd_ukern_mac + '\n' + ("#" * 80) + '\n'
        mac_logs += JU.get_cli_cmd(dev, cmd_ukern_mac)
        for pfe in fpc.pfe_list:
            cmd_hw_mac = 'request pfe execute command "sh l2metro {1} mac hw" target fpc{0}'.format(fpc.slot, pfe)
            mac_logs += ("#" * 80) + '\n' + cmd_hw_mac + '\n' + ("#" * 80) + '\n'
...
def main():
    ...
    while fault is not True:
        ...
        for router in dev_list:
            ...
            logging.log(25, '-' * 80)
            log_name = dev.facts['hostname'] + '.mac'
            if iteration == 1:
...
#                get_hw_mac_list(dev, fpc_list, broken_instances)
                fault = True
                get_mac_data(dev, fpc_list, log_name)
            else:
                logging.log(25, 'Found no MAC stuck events on [{}]. Happy days!'.format(dev.facts['hostname']))
                logging.log(25, 'Removing old log files ......')
                if fault is False:
                    try:
                        os.remove(log_name)
                    except:
                        logging.log(25, 'Could not find old log files ......')
                    get_mac_data(dev, fpc_list, log_name)
        print '\n'
        if fault is True:
